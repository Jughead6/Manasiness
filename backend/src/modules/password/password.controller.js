...
import { requireLoginPassword, requirePassword, requirePasswordMatch, requirePositiveInteger } from "../../utils/validators/index.js"

export async function editPassword(req, res, next) {
    try {
        const storeId = requirePositiveInteger(req.store.storeId, "store_id")
        const currentPassword = requireLoginPassword(req.body.currentPassword, "currentPassword")
